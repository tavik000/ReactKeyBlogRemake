...
export function LocaleProvider({ children, inLocale, inLang, inDict }: LocaleContextProviderProps) {
  const [locale] = useState(inLocale);
  const [lang] = useState(inLang);
  const [dict] = useState(inDict);

  return <LocaleContext.Provider value={{ locale, lang, dict }}>{children}</LocaleContext.Provider>;
}
